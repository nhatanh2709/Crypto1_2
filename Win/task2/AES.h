#ifndef AES_H
#define AES_H
#include <bits/stdc++.h>
using namespace std;
std::vector<uint8_t> S_BOX = {
    };

std::vector<std::vector<uint8_t>> RCON = {
    {0x01, 0x00, 0x00, 0x00},
    {0x02, 0x00, 0x00, 0x00},
    {0x04, 0x00, 0x00, 0x00},
    {0x08, 0x00, 0x00, 0x00},
    {0x10, 0x00, 0x00, 0x00},
    {0x20, 0x00, 0x00, 0x00},
    {0x40, 0x00, 0x00, 0x00},
    {0x80, 0x00, 0x00, 0x00},
    {0x1B, 0x00, 0x00, 0x00},
    {0x36, 0x00, 0x00, 0x00}};

std::vector<uint8_t> INV_S_BOX = {
    };

class AES
{

public:
    std::vector<std::vector<uint8_t>> round_keys;

    std::vector<uint8_t> sub_word(std::vector<uint8_t> key)
    {
        std::vector<uint8_t> result;
        for (auto index : key)
        {
            result.push_back(S_BOX[index]);
        }
        return result;
    }

    std::vector<uint8_t> rot_word(std::vector<uint8_t> key)
    {
        std::rotate(key.begin(), key.begin() + 1, key.end());
        return key;
    }

    void sub_bytes(std::vector<std::vector<uint8_t>> &state)
    {
        for (int i = 0; i < 4; ++i)
        {
            for (int j = 0; j < 4; ++j)
            {
                uint8_t row = state[i][j] / 0x10;
                uint8_t col = state[i][j] % 0x10;
                state[i][j] = S_BOX[16 * row + col];
            }
        }
    }

    void inv_sub_bytes(std::vector<std::vector<uint8_t>> &state)
    {
        for (int i = 0; i < 4; ++i)
        {
            for (int j = 0; j < 4; ++j)
            {
                uint8_t row = state[i][j] / 0x10;
                uint8_t col = state[i][j] % 0x10;
                state[i][j] = INV_S_BOX[16 * row + col];
            }
        }
    }
    void shift_rows(std::vector<std::vector<uint8_t>> &state)
    {
        uint8_t temp = state[1][0];
        state[1][0] = state[1][1];
        state[1][1] = state[1][2];
        state[1][2] = state[1][3];
        state[1][3] = temp;
        temp = state[2][0];
        state[2][0] = state[2][2];
        state[2][2] = temp;
        temp = state[2][1];
        state[2][1] = state[2][3];
        state[2][3] = temp;
        temp = state[3][0];
        state[3][0] = state[3][3];
        state[3][3] = state[3][2];
        state[3][2] = state[3][1];
        state[3][1] = temp;
    }
    void inv_shift_rows(std::vector<std::vector<uint8_t>> &state)
    {
        std::rotate(state[1].rbegin(), state[1].rbegin() + 1, state[1].rend());
        std::rotate(state[2].rbegin(), state[2].rbegin() + 2, state[2].rend());
        std::rotate(state[3].rbegin(), state[3].rbegin() + 3, state[3].rend());
    }
    void mix_columns(std::vector<std::vector<uint8_t>> &state)
    {
        auto const gmul = [&](uint8_t a, uint8_t b)
        {
            uint8_t p = 0;
            for (int counter = 0; counter < 8; counter++)
            {
                if ((b & 1) != 0)
                {
                    p ^= a;
                }
                bool hi_bit_set = (a & 0x80) != 0;
                a <<= 1;
                if (hi_bit_set)
                {
                    a ^= 0x1b;
                }
                b >>= 1;
            }
            return p % 256;
        };
        for (int i = 0; i < 4; ++i)
        {
            std::vector<uint8_t> col(4);
            for (int j = 0; j < 4; ++j)
            {
                col[j] = state[j][i];
            }
            state[0][i] = gmul(0x02, col[0]) ^ gmul(0x03, col[1]) ^ col[2] ^ col[3];
            state[1][i] = col[0] ^ gmul(0x02, col[1]) ^ gmul(0x03, col[2]) ^ col[3];
            state[2][i] = col[0] ^ col[1] ^ gmul(0x02, col[2]) ^ gmul(0x03, col[3]);
            state[3][i] = gmul(0x03, col[0]) ^ col[1] ^ col[2] ^ gmul(0x02, col[3]);
        }
    }
    void inv_mix_columns(std::vector<std::vector<uint8_t>> &state)
    {
        auto const gmul = [&](uint8_t a, uint8_t b)
        {
            uint8_t p = 0;
            for (int counter = 0; counter < 8; counter++)
            {
                if ((b & 1) != 0)
                {
                    p ^= a;
                }
                bool hi_bit_set = (a & 0x80) != 0;
                a <<= 1;
                if (hi_bit_set)
                {
                    a ^= 0x1b;
                }
                b >>= 1;
            }
            return p % 256;
        };
        for (int i = 0; i < 4; ++i)
        {
            std::vector<uint8_t> col(4);
            for (int j = 0; j < 4; ++j)
            {
                col[j] = state[j][i];
            }
            state[0][i] = gmul(0x0E, col[0]) ^ gmul(0x0B, col[1]) ^ gmul(0x0D, col[2]) ^ gmul(0x09, col[3]);
            state[1][i] = gmul(0x09, col[0]) ^ gmul(0x0E, col[1]) ^ gmul(0x0B, col[2]) ^ gmul(0x0D, col[3]);
            state[2][i] = gmul(0x0D, col[0]) ^ gmul(0x09, col[1]) ^ gmul(0x0E, col[2]) ^ gmul(0x0B, col[3]);
            state[3][i] = gmul(0x0B, col[0]) ^ gmul(0x0D, col[1]) ^ gmul(0x09, col[2]) ^ gmul(0x0E, col[3]);
        }
    }

    void add_round_key(std::vector<std::vector<uint8_t>> &state, uint8_t round_number)
    {
        for (int i = 0; i < 4; ++i)
        {
            for (int j = 0; j < 4; ++j)
            {
                state[j][i] ^= round_keys[round_number * 4 + i][j];
            }
        }
    }
    std::vector<std::vector<uint8_t>> key_expansion_128(std::string key)
    {
        uint8_t key_size = 16;
        uint8_t key_words = 4;
        std::vector<uint8_t> keyuint8_ts;

        for (char &c : key)
        {
            keyuint8_ts.push_back(static_cast<uint8_t>(c));
        }

        for (int i = 0; i < key_size; i += 4)
        {
            round_keys.push_back(std::vector<uint8_t>(keyuint8_ts.begin() + i, keyuint8_ts.begin() + i + 4));
        }

        for (int i = key_words; i < 44; ++i)
        {
            std::vector<uint8_t> temp = round_keys[i - 1];
            if (i % key_words == 0)
            {
                temp = sub_word(rot_word(temp));
                for (int j = 0; j < 4; ++j)
                {
                    temp[j] ^= RCON[(i - key_words) / key_words][j];
                }
            }
            std::vector<uint8_t> new_word;
            for (int j = 0; j < 4; ++j)
            {
                new_word.push_back(round_keys[i - key_words][j] ^ temp[j]);
            }
            round_keys.push_back(new_word);
        }
        return round_keys;
    }
    void key_expansion(const std::string &key)
    {
        uint8_t length = (uint8_t)key.size() * 8;
        if (length == 128)
        {
            round_keys = key_expansion_128(key);
        }
        else
        {
            throw std::invalid_argument("Invalid key length. Supported lengths are 128 bits.");
        }
    }

    std::vector<uint8_t> encrypt(std::vector<uint8_t> data, std::string key)
    {

        key_expansion(key);
        std::vector<std::vector<uint8_t>> state;
        for (std::vector<uint8_t>::size_type i = 0; i < data.size(); i += 4)
        {
            state.push_back(std::vector<uint8_t>(data.begin() + i, data.begin() + i + 4));
        }
        uint8_t num_rounds = (key.size() == 16) ? 10 : (key.size() == 24) ? 12
                                                                          : 14;
        add_round_key(state, 0);
        for (int round = 1; round < num_rounds; ++round)
        {
            sub_bytes(state);
            shift_rows(state);
            mix_columns(state);
            add_round_key(state, round);
        }
        sub_bytes(state);
        shift_rows(state);
        add_round_key(state, num_rounds);
        std::vector<uint8_t> result;
        for (auto &row : state)
        {
            result.insert(result.end(), row.begin(), row.end());
        }

        return result;
    }

    std::vector<unsigned char> decrypt(std::vector<unsigned char> ciphertext, std::string key)
    {
        if (round_keys.empty())
        {
            round_keys = key_expansion_128(key);
        }
        std::vector<std::vector<uint8_t>> state;
        for (std::vector<unsigned char>::size_type i = 0; i < ciphertext.size(); i += 4)
        {
            state.push_back(std::vector<uint8_t>(ciphertext.begin() + i, ciphertext.begin() + i + 4));
        }
        uint8_t num_rounds = (key.size() == 16) ? 10 : (key.size() == 24) ? 12 : 14;

        add_round_key(state, num_rounds);
        for (int round = num_rounds - 1; round > 0; --round)
        {
            inv_shift_rows(state);
            inv_sub_bytes(state);
            add_round_key(state, round);
            inv_mix_columns(state);
        }
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, 0);

        std::vector<unsigned char> result;

        for (auto &row : state)
        {
            result.insert(result.end(), row.begin(), row.end());
        }

        return result;
    }
};

#endif
